        ...
        if file.is_file(): # and file.name[-4:]=='.jpg':
            ...
            with open (file_path, "r") as stream:
                ...
                if cropimage is not None:
                    ...
                    #print("CWD:", os.getcwd())
                    save_path = os.path.join(outputfolder, file.name) # specify the name of the file to be saved
...
def scrap():
    ...
    if form_2.validate_on_submit():
        ...
        for i, img_uri in enumerate(images_uri_list): # Image URL. Either Full URL or website relative URL
            if img_uri.find('http')==-1:  # If NOT a full URL , we rebuild the complete image URL adding the base URL 
        ...
        for img_url in images_uri_list:  # Browse the image URL
            ...
            if r.status_code == 200 and pic_counter<=ALLOWED_SCRAPPED_IMAGES:   #200 status code = OK | we process 30 pictures max
                print('Image URL status code 200')
                # PREPARING IMAGE RECORDING TO THE LOCAL FOLDER
                name = img_url.split('/')[-1]   # Retrieving the image NAME from the URL
                    ...
                    with open(filepath, 'wb') as f: # RECORD IMAGE
                        ...
                        try:
                            ...
                            if num_detected_faces >0 and num_detected_faces<=1: # This version only predicts for 1 face/picture
                                ...
                                pic_counter+=1 
                                #prepare df inputs 
                                table.append([name,img_url, None]) # DELETE BUTTON: , filepath.split('/')[-1]]) #function to detect face => emotion
                            else:
                                os.remove(filepath)
                        except Exception:
                            #corrupted file, remove anyway
        ...
        print("LIST OF CROPPED IMG TO PREDICT: ", croppics_path)
        list_names = [(table_row, row[0]) for table_row, row in enumerate(table)]
        ...
        if len(table)==0:
            table.append(['sorry these images can not be process',' ','No prediction'])
            # Tranform df to html
            df = pd.DataFrame(table, columns=['Title', 'Image', 'Emotion',]) ## DELETE BUTTON: 'Delete'])
